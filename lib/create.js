...
module.exports = async(projectName) => {
    ...
    try {
        ...
        await copyBaseTemplate(projectPath) // 复制基础模板
        await generateFrameWork(projectPath) // 生成框架相关文件
        await generateUI(projectPath) // 生成UI库相关文件
        await generateLint(projectPath) // 生成规范工具相关文件
        ...
        ${chalk.cyan(shouldInstall?'npm run dev':'npm install && npm run dev')}
        }`)

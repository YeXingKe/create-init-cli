...
async function saveAsTemplate(currentConfig,projectName){
   const {saveTemplate} = await inquirer.prompt([
    {
        type:'confirm',
        name:'saveTemplate',
        mesage:'Would you like to save the current settings as a template?',
        default:true
    }
   ])
   ...
   await fs.writeFile(configPath,JSON.stringify(currentConfig,null,2)) // 写入配置
}
...
async function findTemplate(templateName,configJSON){
   ...
   const readJSON = fs.readJSON(cacheTemplatePath)
   if (fs.existsSync(cacheTemplatePath) && readJSON === configJSON) {
    // 检查目录是否存在
    ...
    fs.ensureDir(cacheTemplatePath)
  }
